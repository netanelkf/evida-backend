  ...
  asyncHandler(async (req, res) => {
    ...
    const user = await db.transaction(async (trx) => {
      const [newUser] = await trx('users')
        .insert({ email, password_hash, name, phone, role: role || 'user' })
        .returning('*');
      const thresholdRows = DEFAULT_THRESHOLDS.map((t) => ({ ...t, user_id: newUser.id }));
      await trx('thresholds').insert(thresholdRows);
      return newUser;
    });
